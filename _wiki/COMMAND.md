...
| `npm run predev` | Create the root `.env` from `.env.example` when it does not exist. Runs automatically before `dev`. |
| `npm run dev` | Ensure the root `.env` exists, load it, and start workspace development tasks through Turborepo. |
